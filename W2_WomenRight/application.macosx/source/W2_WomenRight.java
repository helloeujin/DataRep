...
public class W2_WomenRight extends PApplet {
...
PFont mono;
float unit_x = 1280*0.05f;
float unit_y = 720*0.1f;
...
//  textAlign(CORNER);
//  text("WOMEN'S RIGHT TO VOTE", unit_x, unit_y*1.46);
//  text("YEAR WOMEN GOT RIGHT TO VOTE", unit_x, unit_y*1.46);
  textAlign(CENTER);
  text("WOMEN GOT RIGHT TO VOTE", width/2, unit_y*1.45f);
...
class Continent {
  ...
  float unit_x = width*0.05f;
  float unit_y = height*0.1f;
  float easing = 0.05f;
  ...
  public void displayCircle(float ratio_af_, float ratio_as_, float ratio_er_, float ratio_nt_, float ratio_st_, float ratio_oc_) {
    textAlign(CENTER, CENTER);
    noStroke();
    
    ratio_af = PApplet.parseInt(ratio_af_);
    ratio_as = PApplet.parseInt(ratio_as_);
    ratio_er = PApplet.parseInt(ratio_er_);
    ratio_nt = PApplet.parseInt(ratio_nt_);
    ratio_st = PApplet.parseInt(ratio_st_);
    ratio_oc = PApplet.parseInt(ratio_oc_);
    
    for(int i = 0; i < 6; i ++) {
      float cent_x = unit_x + d/2 + i*d + i*gap;
      float cent_y = 3.8f*unit_y;
      ...
      if(i == 0) {
        ...
        arc(cent_x, cent_y, d*0.88f, d*0.88f, 0, th_0);
      }
      if(i == 1) {
        textSize(32);
        ...
        arc(cent_x, cent_y, d*0.88f, d*0.88f, 0, th_1);
      }
      if(i == 2) {
        textSize(32);
        ...
        arc(cent_x, cent_y, d*0.88f, d*0.88f, 0, th_2);
      }
      if(i == 3) {
        textSize(32);
        ...
        arc(cent_x, cent_y, d*0.88f, d*0.88f, 0, th_3);
      }
      if(i == 4) {
        textSize(32);
        ...
        arc(cent_x, cent_y, d*0.88f, d*0.88f, 0, th_4);
      }
      if(i == 5) {
        textSize(32);
        ...
        arc(cent_x, cent_y, d*0.88f, d*0.88f, 0, th_5);
      } 
...
class Data {
  ...
  public void display(int i_, int selectedYear_) {
    num = i_;
...
//      fill(0);
  ...
  public void displayScrollAuto() {
    ...
    int tempYear = PApplet.parseInt(map(temp_x, min_x, max_x, 1890, 2010));
...
//      textSize(11);
  ...
  public void loadData() {
    for (TableRow row : table.rows()) {
      ...
      if (right2vote.length() == 4) { //right2vote.equals("..") == false
        voteYear = PApplet.parseInt(right2vote);
        ...
        int firstYear = PApplet.parseInt(years[0]);
        int lastYear = PApplet.parseInt(years[numYear - 1]);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "W2_WomenRight" };
    if (passedArgs != null) {
