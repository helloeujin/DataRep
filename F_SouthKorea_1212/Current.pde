class Current {
  ...
  void setpos() {
    ...
    float h = map(total, 81, 3060, 20, 180);
    for(int i = 0; i < total; i++) {
      xoff = xoff + .5;
      yoff = yoff + .5;
...
//      textAlign(LEFT);
//      textSize(40);
//      text("Former South Korean President Roh Moo-Hyun committed suicide Saturday", 50, height*0.25);
//      text("by leaping to his death from a hill behind his house, the government announced.", 50, height*0.32);
//      text("He was 62.", 50, height*0.39);
      
      textAlign(CENTER);
      textSize(48);
      text("Number of suicides by age group and gender", width/2, height*0.34);
    }
  }
}
